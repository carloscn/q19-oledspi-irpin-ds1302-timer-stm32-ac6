...
void    UART1_INIT( UART_T* self, uint32 baud )
{
...
void    UART2_INIT( UART_T* self, uint32 baud )
{
    self->init          =   &uart_init;
    self->pin_set       =   &uart_pin_set;
    self->write_byte    =   &uart_write_byte;
...
void    UART3_INIT( UART_T* self , uint32 baud)
{
    self->init          =   &uart_init;
    self->pin_set       =   &uart_pin_set;
    self->write_byte    =   &uart_write_byte;
...
void    uart_init( struct uart_t *self )
{
    ...
    if( self->uart_cfg.uart_id  ==  UART_ID_1 ) {
        ...
        irq = USART3_IRQn;
    }else if( self->uart_cfg.uart_id == UART_ID_4 ){
        // irq = USART3_IRQn;
    }else if( self->uart_cfg.uart_id == UART_ID_5 ){
        // irq = USART3_IRQn;
    }
...
extern void    bluetooth_message_deal( BYTE byte_data );
...
void USART2_IRQHandler( void )
{
    ...
    if ( USART_GetITStatus(USART2,USART_IT_RXNE) != RESET ) {
        ...
        uart_buffer[ uart_int_count ] = USART_ReceiveData(USART2);
        uart_int_count ++;
        if( uart_int_count >= UART_RECV_BUFFER_MAX ) {
            uart_int_count = 0;
            uart_full_flag  =   true;
        }
